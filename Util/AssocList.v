...
Definition updates_AL{X Y}`{Eq X} : list (X * Y) -> AssocList X Y -> AssocList X Y := fold_right (fun '(x,y) ps => update_AL x y ps).

Lemma In_map_cons{X Y} : forall (x x' : X)(y : Y)(qs : AssocList X Y), In x (map fst ((x',y)::qs)) -> x <> x' -> In x (map fst qs).
Proof.
  intros.
  simpl in H.
  destruct H.
...
Fixpoint lookup{X Y}`{Eq X}(x : X)(ps : AssocList X Y) : option Y :=
  match ps with
  | [] => None
  | (x',y)::qs => if x =dec x' then Some y else lookup x qs
  end.